...
library LibRegister {
  ...
  function delegatecallRegisterFunction(address target) internal {
    (bool success, bytes memory resultData) = target.delegatecall(abi.encodeWithSignature("register()"));
    require(success && resultData.length == 32 && abi.decode(resultData, (bytes4)) == REGISTER_SUCCESS, "RegisterFailedError");
  }
}